...
class ScaleLoss(nn.Module):
    ...
    def forward(self, pred_scale, gt_scale):
        ...
        if torch.isnan(loss):
            print(f'Found a NaN loss, type {self.loss=} from {self.__class__.__name__}')
            print(f'Inputs were use to calculate loss: {pred_scale.numel()=}, {gt_scale.numel()=}')
            return torch.tensor(1e-5, device=pred_scale.device)
        return loss
...
class InplaneLoss(nn.Module):
    ...
    def forward(self, pred_cos_sin, gt_cos_sin):
        ...
        if torch.isnan(loss):
            print(f'Found a NaN loss, type {self.loss=} from {self.__class__.__name__}')
            print(f'Inputs were use to calculate loss: {pred_cos_sin.numel()=}, {gt_cos_sin.numel()=}')
            return torch.tensor(1e-5, device=pred_cos_sin.device)
        return loss
